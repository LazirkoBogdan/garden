...
export async function createScene() {
  ...
  const particleExplosion = await initParticleExplosion();
  if (particleExplosion.particleSystem) {
    particleExplosion.particleSystem.emitter.position.set(0, 5, 0);
    batchSystem.addSystem(particleExplosion.particleSystem);
  }

  const gameGrid = new GameGrid(GRID, interactionManager, sceneManager, particleExplosion);
...
async function initParticleExplosion() {
  const particleExplosion = new ParticleExplosion('assets/images/smoke.png');
  await particleExplosion.createExplosion();
